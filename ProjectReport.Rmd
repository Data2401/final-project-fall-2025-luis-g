...
meteroite <- meteroite %>% 
  ...
  drop_na(reclat)%>% 
  select(name, id, type, nametype, class, mass, fall, year, reclat, reclong)
...
meteroite %>% ggplot(aes(x = type, y = mass)) + geom_boxplot()
meteroite %>% ggplot(aes(x = type, y = log(mass))) + geom_boxplot()
```


```{r}
meteroite %>%
  filter(hemisphere != "Equator") %>% 
  count(type, fall, hemisphere) %>%                          
  group_by(type, hemisphere) %>%                              
  mutate(pct = n / sum(n)) %>%                    
  ggplot(aes(y = type, x = pct, fill = fall)) +
...
```{r}
meteroite %>% group_by(hemisphere, year) %>% summarise(n = n()) %>% ggplot(aes(x = year, y = n, color = hemisphere)) + geom_line()
```
